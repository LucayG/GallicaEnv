...
print("Vous allez pouvoir faire une requête sur Gallica avec une liste de vocabulaire !\n")

''' Term list '''

#ask whether a UNIMARC file or a text file will be used
answer = input("Utiliser un fichier UNIMARC en entrée ? (y si oui, n (ou rien) sinon)\n")

#if the file is UNIMARC, transform it into a simple text file
if answer == 'y' or answer == 'Y':
	try :
		fil_one = input("Nom du fichier UNIMARC (sans l'extension .txt)\n")
	except FileNotFoundError :
		print("Nous n'avons pas trouvé le fichier ! Vérifiez l'orthographe et son emplacement.\n")
		fil_one = input("Nom du fichier UNIMARC (sans l'extension .txt)\n")
	extract(fil_one)
    
#if the file is already a text file, proceed as usual
else: 
	try :
		fil_one = input("Nom du fichier texte (sans l'extension .txt)\n")
	except FileNotFoundError :
		print("Nous n'avons pas trouvé le fichier ! Vérifiez l'orthographe et son emplacement.\n")
		fil_one = input("Nom du fichier texte (sans l'extension .txt)\n")
...
chaans = input('Verifier chaque terme un par un (y si oui, sinon) ?\n')

#if deciding to check the list's words, read them one by one and validate or reject
if chaans == "y" or chaans == "Y" :
    with open('ramtolist1.txt', 'r', encoding='UTF-8') as fil:
        for line in fil:
            try:
                line = line.strip()
                ans = str(input("Inserer " + line + " ? (y pour inserer, n (ou rien) pour rejeter)\n"))
                if ans == "y" or ans == "Y":    
                    lst1.append(line)
                else:
                    continue
            except EOFError:
                print("EOFE ERROR ! Certains IDE ne fonctionnent pas avec ce programme.")
                break
    lstf = lst1

#if not checking the words, proceed as usual and combine words into couples
else:
    with open('ramtolist1.txt', 'r', encoding='UTF-8') as fil:
        for line in fil:
            line = line.strip()
...
for i in lstf:
    ...
    if __name__ == '__main__':
        ...
        # info from the gallica documents
        if nbx != -1:
...
with open('arkgal2.txt', 'r') as arkg, open('ramtolist1.txt', 'r') as ram:
    ...
    for arkline in lines:        
        ...
        narkline = arkline.split("/")[-2]
        print("Scoring document : " + str(narkline))
        print('Arkline   :  ', arkline)
        #for each single word
        for ramline in linez :
            ramline = re.sub("[ ]", "+", ramline)
            ramline = '"' + ramline.lower().strip() + '"'
            print("Scoring word : " + str(ramline))
            link = initial + narkline + end + urllib.parse.quote_plus(ramline)
            ...
            with open('pages.txt', 'a', encoding=('utf-8')) as pag, open('content.txt', 'a', encoding='utf-8') as cont:
                ...
                for k in tlist :
                    ...
                    rampage = ramline.strip('"')
                    link = initial + narkline + end + urllib.parse.quote_plus(rampage) + onpage
                    page = requests.get(link)
                    page.encoding = "UTF-8"
                    try :
    ...
    with open('tables.xml', 'w', encoding = 'utf-8', newline='') as tab:
        ...
        for key, value in realdict.items():
            if value[3] <= 0.5 :
                ...
                trawpage = str(re.sub(r'PAG_','', key))[:4]     #2360_bpt6k98002169
                rawpage = str(re.sub(r'[bpt_v]','', trawpage))  #2360
...
Path("ramtolist1.txt").rename("Depot_Gallica{}/Liste_1.txt".format(str(folnb)))
...
if answer == 'y' or answer == 'Y':
    Path(fil_one + '.txt').rename("Depot_Gallica{}/Doc_Unimarc_1.txt".format(str(folnb)))

#remove residual text and xml files
for file in os.scandir():
    if (file.name.endswith(".txt") and not file.name.startswith(fil_one)) or file.name.endswith(".xml"): 
        os.unlink(file)
